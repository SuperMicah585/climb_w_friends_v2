...
import { ClimbsTableResponse,ClimbWithDependencies } from "../../types/interfaces";
type SpecialGrade = 'VEASY' | 'VBASIC' | 'VBEGINNER' | 'VB' | 'EASY' | 'BEGINNER' | 'BASIC';
...
const cleanGradeString = (gradeStr: string | null | undefined): string => {
  ...
  gradeStr = gradeStr.toUpperCase()
    .replace(/\s+/g, '')
    .replace(/^V-/, 'V');  // Handle "V-Easy" -> "VEASY"
...
const parseGrade = (gradeStr: string | null | undefined): number => {
  ...
  if (gradeStr.startsWith('V')) {
    ...
    return (gradeStr as SpecialGrade in SPECIAL_GRADES) ? 
      SPECIAL_GRADES[gradeStr as SpecialGrade] : 0;
  }
...
const groupByGrade = (input:any) =>{
    const combinedArray = [{rating:'v0-v3',count:0},{rating:'v4-v6',count:0},{rating:'v7+',count:0}]
...
const sortByGradeDesc = (a: ClimbWithDependencies, b: ClimbWithDependencies): number => {
